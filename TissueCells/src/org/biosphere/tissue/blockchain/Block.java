...
public class Block
{
  ...
  /**
   * Creates one instance of e Block based on one cell identifier and a flat String
   * <p>
   * It is mainly intended to be used in the interactions between Cells, when propagating the local append of one block to the chain.
   * 
   * @param CellId    the unique identified of the Cell that added the Block to the chain
   * @param flatBlock the flat String that represents the Block content
   */   
  Block(String flatBlock,Chain chain) throws BlockException
  {
    super();
    logger = new Logger();
    logger.debug("Block.Block()","Creating new Block: cell("+flatBlock.split(":")[1]+") prev block ID("+flatBlock.split(":")[2]+") block ID("+flatBlock.split(":")[3]+")");    
   ...
   * <p>
   * It is mainly intended to be used while adding a Cell to the chain locally.
   * 
   * @param CellId the unique identified of the Cell that added the Block to the chain, mostly the local Cell.
   */
   ...
   * 
   * @param acceptanceVotes ArrayList<Vote>
   */
  ...
  public synchronized void addVote(Vote vote)
  {
     ...
     {
       logger.debug("Block.addVote()","Adding vote: "+vote.getCellID()+" voted: ("+vote.isAccepted()+") for block "+getBlockID());
   ...
   * 
   * @param cellName Name of the cell to be checked
   ...
   * 
   * @param timestamp Date representing the Block creation timestamp
   */
  ...
  /**
   * Defines the Cell ID of this Block, should only be called when creating the Block
   * 
   * @param cellId
   */
   ...
   * 
   * @param prevHash The previouos Block Hash to be set in the prevHash attribute
   */
   ...
   * 
   * @param payload The payload to be defined 
   */
  ...
  String toFlat()
  {
    return getChainPosition()+":"+getTimestamp().getTime()+":"+getCellID()+":"+getPrevBlockID()+":"+getBlockID()+":"+getPrevHash()+":"+getBlockHash()+":"+Base64.toBase64String(getPayload().getBytes(StandardCharsets.UTF_8));
  }
   ...
   * 
   * @param prevBlockId The ID of the block that is the previous for this block in the chain
   */
   ...
   * 
   * @param blockId String representing this Block ID
   */
   ...
   * 
   * @param chain The chain that this block belongs to
   */
   ...
   * 
   * @param nextBlockID the id of the next block
   */
  ...
  /**
   * Set the next block IDs array list (used during the instantiation of a new block)
   * 
   * @param nextBlockIDs
   */
