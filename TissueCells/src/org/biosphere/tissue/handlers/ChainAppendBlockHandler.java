...
public class ChainAppendBlockHandler implements CellHTTPHandlerInterface
{
  ...
  public void handle(HttpExchange t)
  {
    try
    {
      ...
      logger.debugAppBlock("ChainAppendBlockHandler.handle()", "Cell "+cell.getCellName()+" request from: " + partnerCell);
      String request = RequestUtils.getRequestAsString(t.getRequestBody());
      boolean accepted = cell.getChain().appendBlock(request);
      logger.debugAppBlock("ChainAppendBlockHandler.handle()", "Block accepted by "+cell.getCellName()+":"+accepted);
